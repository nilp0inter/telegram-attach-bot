...
def parse_args(args):
    ...
    Namespace(token='foo', chat=['bar', 'baz'], max_entry_size=None, max_total_size=None, include_ext=None, metadata=None, path='somepath')

    >>> parse_args(["--token", "foo", "--chat", "bar", "--max-entry-size", "100 MB", "--max-total-size", "2 GB", "somepath"])
    Namespace(token='foo', chat=['bar'], max_entry_size=100000000, max_total_size=2000000000, include_ext=None, metadata=None, path='somepath')

    >>> parse_args(["--token", "foo", "--chat", "bar", "--include-ext", "jpg", "--include-ext", "pdf", "somepath"])
    Namespace(token='foo', chat=['bar'], max_entry_size=None, max_total_size=None, include_ext=['jpg', 'pdf'], metadata=None, path='somepath')

    >>> parse_args(["--token", "foo", "--chat", "bar", "--metadata", "key1=value1", "--metadata", "key2=this is value=2", "somepath"])
    Namespace(token='foo', chat=['bar'], max_entry_size=None, max_total_size=None, include_ext=None, metadata={'key1': 'value1', 'key2': 'this is value=2'}, path='somepath')
